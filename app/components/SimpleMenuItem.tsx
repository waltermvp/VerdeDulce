 ...
 */
export const SimpleMenuItem = observer(function SimpleMenuItem(props: SimpleMenuItemProps) {
  const { style, title, description, price, index } = props
  const $styles = [$container, style]

  const backgroundColor = index % 2 === 0 ? colors.palette.greenFont : colors.palette.neutral100
  const fontColor = index % 2 === 0 ? colors.palette.neutral100 : colors.palette.greenFont
  ...
  return (
    <View style={[$styles, { backgroundColor }]}>
      <View style={{ flexDirection: "column", flex: 4.4 }}>
        ...
        <Text style={[styles2.description, { color: fontColor }]}>{description}</Text>
      </View>
      <View style={{ flex: 1 }}>
